...
async function main() {
	if (!testUuid(extensionConfig.uuid)) {
		...
		// @ts-expect-error remove default when generated by TS
	...
	await fs.writeFile(path.join(__dirname, '../dist', `${extensionConfig.name}_v${extensionConfig.version}.eext`), zipBuffer);
}
